      ...
      fetchAll(lang)
        .then((data) => {
          ...
          for (const key of keys) {
            ...
            // Every block is a child of the post body (cooked), guarded per key.
            if (cooked.querySelector(`:scope > [data-thb-key="${cssEscape(key)}"]`)) {
              continue;
            }
            if (slot === "top") {
              // Strip at the TOP of the post body — contained within the post
              // card, above the content. (Title-area placement broke alignment.)
              if (!b.html) continue;
